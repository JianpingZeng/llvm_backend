...
//TODO: Enable below function definitions
...
Cse523TargetLowering::IsEligibleForTailCallOptimization(SDValue Callee,
    ...
    // If the function return type is cse523_fp80 and the callee return type is not,
    // then the FP_EXTEND of the call result is not a nop. It's not safe to
    // perform a tailcall optimization here.

    assert(0);
    //if (CallerF->getReturnType()->isCse523_FP80Ty() && !RetTy->isCse523_FP80Ty())
    //    return false;
...
static SDValue getVShift(bool isLeft, EVT VT, SDValue SrcOp,
    ...
    EVT ShVT = MVT::v2i64;
    unsigned Opc = isLeft ? Cse523ISD::VSHLDQ : Cse523ISD::VSRLDQ;
...
static SDValue LowerVectorBroadcast(SDValue Op, const Cse523Subtarget* Subtarget,
        SelectionDAG &DAG) {
    if (!Subtarget->hasFp256())
        return SDValue();
    ...
    switch (Op.getOpcode()) {
        ...
        case ISD::VECTOR_SHUFFLE: {
                                      ...
                                      if (Sc.getOpcode() != ISD::SCALAR_TO_VECTOR &&
                                          ...
                                          return DAG.getNode(Cse523ISD::VBROADCAST, dl, VT, Sc);
                                      }
    ...
    if (ConstSplatVal && Subtarget->hasInt256()) {
        ...
        if (ScalarSize == 32 || (IsGE256 && ScalarSize == 64)) {
            ...
            return DAG.getNode(Cse523ISD::VBROADCAST, dl, VT, Ld);
        }
    }
...
Cse523TargetLowering::LowerBUILD_VECTOR(SDValue Op, SelectionDAG &DAG) const {
    ...
    if (ISD::isBuildVectorAllZeros(Op.getNode())) {
        ...
        // and 2) ensure that i64 scalars are eliminated on cse523-32 hosts.
    ...
    if (NumNonZero == 1) {
        ...
        // If this is an insertion of an i64 value on cse523-32, and if the top bits of
        ...
        if (NumElems == 2 && Idx == 1 &&
                Cse523::isZeroNode(Op.getOperand(0)) &&
                !Cse523::isZeroNode(Op.getOperand(1))) {
...
LowerVECTOR_SHUFFLEtoBlend(ShuffleVectorSDNode *SVOp,
        const Cse523Subtarget *Subtarget, SelectionDAG &DAG) {
    ...
    SDValue Ret = DAG.getNode(Cse523ISD::BLENDI, dl, BlendVT, V1, V2,
...
static SDValue
LowerVECTOR_SHUFFLEv8i16(SDValue Op, const Cse523Subtarget *Subtarget,
        SelectionDAG &DAG) {
    ShuffleVectorSDNode *SVOp = cast<ShuffleVectorSDNode>(Op);
    SDValue V1 = SVOp->getOperand(0);
    SDValue V2 = SVOp->getOperand(1);
    ...
    if (BestLoQuad >= 0 || BestHiQuad >= 0) {
        ...
        if ((pshufhw && InOrder[0]) || (pshuflw && InOrder[1])) {
            unsigned Opc = pshufhw ? Cse523ISD::PSHUFHW : Cse523ISD::PSHUFLW;
    ...
    if (Subtarget->hasSSSE3()) {
        ...
        V1 = DAG.getNode(Cse523ISD::PSHUFB, dl, MVT::v16i8, V1,
        ...
        V2 = DAG.getNode(Cse523ISD::PSHUFB, dl, MVT::v16i8, V2,
                DAG.getNode(ISD::BUILD_VECTOR, dl,
                    MVT::v16i8, &pshufbMask[0], 16));
    ...
    if (BestLoQuad >= 0) {
        ...
        if (NewV.getOpcode() == ISD::VECTOR_SHUFFLE && Subtarget->hasSSSE3()) {
            ShuffleVectorSDNode *SVOp = cast<ShuffleVectorSDNode>(NewV.getNode());
            NewV = getTargetShuffleNode(Cse523ISD::PSHUFLW, dl, MVT::v8i16,
                    NewV.getOperand(0),
    ...
    if (BestHiQuad >= 0) {
        ...
        if (NewV.getOpcode() == ISD::VECTOR_SHUFFLE && Subtarget->hasSSSE3()) {
            ShuffleVectorSDNode *SVOp = cast<ShuffleVectorSDNode>(NewV.getNode());
            NewV = getTargetShuffleNode(Cse523ISD::PSHUFHW, dl, MVT::v8i16,
                    NewV.getOperand(0),
...
SDValue LowerVECTOR_SHUFFLEv32i8(ShuffleVectorSDNode *SVOp,
    ...
    return DAG.getNode(Cse523ISD::PSHUFB, dl, MVT::v32i8, V1,
            DAG.getNode(ISD::BUILD_VECTOR, dl,
...
static SDValue getVZextMovL(MVT VT, MVT OpVT,
        ...
        const Cse523Subtarget *Subtarget, SDLoc dl) {
    if (VT == MVT::v2f64 || VT == MVT::v4f32) {
        ...
        if (!LD) {
            ...
            if ((ExtVT != MVT::i64 || Subtarget->is64Bit()) &&
                ...
                return DAG.getNode(ISD::BITCAST, dl, VT,
                        DAG.getNode(Cse523ISD::VZEXT_MOVL, dl, OpVT,
                            DAG.getNode(ISD::SCALAR_TO_VECTOR, dl,
                                OpVT,
                                SrcOp.getOperand(0)
                                .getOperand(0))));
...
SDValue getMOVDDup(SDValue &Op, SDLoc &dl, SDValue V1, SelectionDAG &DAG) {
    ...
    return DAG.getNode(ISD::BITCAST, dl, VT,
            getTargetShuffleNode(Cse523ISD::MOVDDUP, dl, MVT::v2f64,
                V1, DAG));
}
...
SDValue getMOVLowToHigh(SDValue &Op, SDLoc &dl, SelectionDAG &DAG,
    ...
    if (HasSSE2 && VT == MVT::v2f64)
        return getTargetShuffleNode(Cse523ISD::MOVLHPD, dl, VT, V1, V2, DAG);

    // v4f32 or v4i32: canonizalized to v4f32 (which is legal for SSE1)
    return DAG.getNode(ISD::BITCAST, dl, VT,
            getTargetShuffleNode(Cse523ISD::MOVLHPS, dl, MVT::v4f32,
...
SDValue getMOVHighToLow(SDValue &Op, SDLoc &dl, SelectionDAG &DAG) {
    ...
    // v4i32 or v4f32
    return getTargetShuffleNode(Cse523ISD::MOVHLPS, dl, VT, V1, V2, DAG);
}
...
SDValue getMOVLP(SDValue &Op, SDLoc &dl, SelectionDAG &DAG, bool HasSSE2) {
    ...
    //  (store (v4f32 (Cse523Movlps (load addr:$src1), VR128:$src2)), addr:$src1)
    //    turns into:
    ...
    if (CanFoldLoad) {
        if (HasSSE2 && NumElems == 2)
            return getTargetShuffleNode(Cse523ISD::MOVLPD, dl, VT, V1, V2, DAG);

        if (NumElems == 4)
            // If we don't care about the second element, proceed to use movss.
            if (SVOp->getMaskElt(1) != -1)
                return getTargetShuffleNode(Cse523ISD::MOVLPS, dl, VT, V1, V2, DAG);
    }
    ...
    // matching to cse523 specific nodes. Note that for the 1st condition all
    ...
    if (HasSSE2) {
        ...
        if (NumElems == 2 || !isMOVLMask(SVOp->getMask(), VT))
            return getTargetShuffleNode(Cse523ISD::MOVSD, dl, VT, V1, V2, DAG);
        return getTargetShuffleNode(Cse523ISD::MOVSS, dl, VT, V1, V2, DAG);
    }

    assert(VT != MVT::v4i32 && "unsupported shuffle type");

    // Invert the operand order and use SHUFPS to match it.
    return getTargetShuffleNode(Cse523ISD::SHUFP, dl, VT, V2, V1,
            getShuffleSHUFImmediate(SVOp), DAG);
}

// Reduce a vector shuffle to zext.
static SDValue LowerVectorIntExtend(SDValue Op, const Cse523Subtarget *Subtarget,
        SelectionDAG &DAG) {
    ...
    return DAG.getNode(ISD::BITCAST, DL, VT,
            DAG.getNode(Cse523ISD::VZEXT, DL, NVT, V1));
}

static SDValue
NormalizeVectorShuffle(SDValue Op, const Cse523Subtarget *Subtarget,
        SelectionDAG &DAG) {
    ShuffleVectorSDNode *SVOp = cast<ShuffleVectorSDNode>(Op);
    MVT VT = Op.getSimpleValueType();
    SDLoc dl(Op);
...
Cse523TargetLowering::LowerVECTOR_SHUFFLE(SDValue Op, SelectionDAG &DAG) const {
    ...
    // 2) Matching of shuffles with known shuffle masks to cse523 target specific
    //    shuffle nodes.
    ...
    if (OptForSize && isUNPCKL_v_undef_Mask(M, VT, HasInt256))
        return getTargetShuffleNode(Cse523ISD::UNPCKL, dl, VT, V1, V1, DAG);
    if (OptForSize && isUNPCKH_v_undef_Mask(M, VT, HasInt256))
        return getTargetShuffleNode(Cse523ISD::UNPCKH, dl, VT, V1, V1, DAG);
    ...
    if (isPSHUFDMask(M, VT)) {
        ...
        if (isMOVDDUPMask(M, VT) && ((VT == MVT::v4f32 || VT == MVT::v2i64)))
            return getTargetShuffleNode(Cse523ISD::MOVLHPS, dl, VT, V1, V1, DAG);

        unsigned TargetMask = getShuffleSHUFImmediate(SVOp);

        if (HasSSE2 && (VT == MVT::v4f32 || VT == MVT::v4i32))
            return getTargetShuffleNode(Cse523ISD::PSHUFD, dl, VT, V1, TargetMask, DAG);

        if (HasFp256 && (VT == MVT::v4f32 || VT == MVT::v2f64))
            return getTargetShuffleNode(Cse523ISD::VPERMILP, dl, VT, V1, TargetMask,
                    DAG);

        return getTargetShuffleNode(Cse523ISD::SHUFP, dl, VT, V1, V1,
                TargetMask, DAG);
    }

    if (isPALIGNRMask(M, VT, Subtarget))
        return getTargetShuffleNode(Cse523ISD::PALIGNR, dl, VT, V1, V2,
    ...
    if (V2IsUndef && isMOVSHDUPMask(M, VT, Subtarget))
        return getTargetShuffleNode(Cse523ISD::MOVSHDUP, dl, VT, V1, DAG);

    if (V2IsUndef && isMOVSLDUPMask(M, VT, Subtarget))
        return getTargetShuffleNode(Cse523ISD::MOVSLDUP, dl, VT, V1, DAG);
    ...
    if (isUNPCKLMask(M, VT, HasInt256))
        return getTargetShuffleNode(Cse523ISD::UNPCKL, dl, VT, V1, V2, DAG);

    if (isUNPCKHMask(M, VT, HasInt256))
        return getTargetShuffleNode(Cse523ISD::UNPCKH, dl, VT, V1, V2, DAG);
    ...
    // Normalize the node to match cse523 shuffle ops if needed
    ...
    if (isPSHUFHWMask(M, VT, HasInt256))
        return getTargetShuffleNode(Cse523ISD::PSHUFHW, dl, VT, V1,
                getShufflePSHUFHWImmediate(SVOp),
                DAG);

    if (isPSHUFLWMask(M, VT, HasInt256))
        return getTargetShuffleNode(Cse523ISD::PSHUFLW, dl, VT, V1,
                getShufflePSHUFLWImmediate(SVOp),
                DAG);

    if (isSHUFPMask(M, VT))
        return getTargetShuffleNode(Cse523ISD::SHUFP, dl, VT, V1, V2,
                getShuffleSHUFImmediate(SVOp), DAG);
    ...
    if (V2IsUndef && isMOVDDUPYMask(M, VT, HasFp256))
        return getTargetShuffleNode(Cse523ISD::MOVDDUP, dl, VT, V1, DAG);

    // Handle VPERMILPS/D* permutations
    if (isVPERMILPMask(M, VT)) {
        if ((HasInt256 && VT == MVT::v8i32) || VT == MVT::v16i32)
            return getTargetShuffleNode(Cse523ISD::PSHUFD, dl, VT, V1,
                    getShuffleSHUFImmediate(SVOp), DAG);
        return getTargetShuffleNode(Cse523ISD::VPERMILP, dl, VT, V1,
                getShuffleSHUFImmediate(SVOp), DAG);
    }

    // Handle VPERM2F128/VPERM2I128 permutations
    if (isVPERM2X128Mask(M, VT, HasFp256))
        return getTargetShuffleNode(Cse523ISD::VPERM2X128, dl, VT, V1,
    ...
    if (V2IsUndef && HasInt256 && isPermImmMask(M, VT, Imm8))
        return getTargetShuffleNode(Cse523ISD::VPERMI, dl, VT, V1, Imm8, DAG);
    ...
    if ((V2IsUndef && HasInt256 && VT.is256BitVector() && NumElems == 8) ||
        ...
        if (V2IsUndef)
            ...
            return DAG.getNode(Cse523ISD::VPERMV, dl, VT,
                    DAG.getNode(ISD::BITCAST, dl, VT, Mask), V1);
        return DAG.getNode(Cse523ISD::VPERMV3, dl, VT, V1,
...
static SDValue LowerEXTRACT_VECTOR_ELT_SSE4(SDValue Op, SelectionDAG &DAG) {
    ...
    if (VT.getSizeInBits() == 8) {
        SDValue Extract = DAG.getNode(Cse523ISD::PEXTRB, dl, MVT::i32,
            ...
            return DAG.getNode(ISD::TRUNCATE, dl, MVT::i16,
                    DAG.getNode(ISD::EXTRACT_VECTOR_ELT, dl, MVT::i32,
                        DAG.getNode(ISD::BITCAST, dl,
                            ...
                            Op.getOperand(0)),
                        Op.getOperand(1)));
        SDValue Extract = DAG.getNode(Cse523ISD::PEXTRW, dl, MVT::i32,
                Op.getOperand(0), Op.getOperand(1));
        SDValue Assert  = DAG.getNode(ISD::AssertZext, dl, MVT::i32, Extract,
                DAG.getValueType(VT));
        return DAG.getNode(ISD::TRUNCATE, dl, VT, Assert);
...
static SDValue ExtractBitFromMaskVector(SDValue Op, SelectionDAG &DAG) {
    ...
    Vec = DAG.getNode(Cse523ISD::VSHLI, dl, VecVT, Vec,
            DAG.getConstant(MaxSift - IdxVal, MVT::i8));
    Vec = DAG.getNode(Cse523ISD::VSRLI, dl, VecVT, Vec,
            DAG.getConstant(MaxSift, MVT::i8));
    return DAG.getNode(Cse523ISD::VEXTRACT, dl, MVT::i1, Vec,
            DAG.getIntPtrConstant(0));
}
...
Cse523TargetLowering::LowerEXTRACT_VECTOR_ELT(SDValue Op,
    ...
    if (!isa<ConstantSDNode>(Idx)) {
        if (VecVT.is512BitVector() ||
            ...
            SDValue Mask = DAG.getNode(Cse523ISD::VINSERT, dl, MaskVT,
                    getZeroVector(MaskVT, Subtarget, DAG, dl),
                    Idx, DAG.getConstant(0, getPointerTy()));
            SDValue Perm = DAG.getNode(Cse523ISD::VPERMV, dl, VecVT, Mask, Vec);
    ...
    if (VT.getSizeInBits() == 16) {
        ...
        MVT EltVT = MVT::i32;
        SDValue Extract = DAG.getNode(Cse523ISD::PEXTRW, dl, EltVT,
                Op.getOperand(0), Op.getOperand(1));
...
static SDValue LowerINSERT_VECTOR_ELT_SSE4(SDValue Op, SelectionDAG &DAG) {
    ...
    if ((EltVT.getSizeInBits() == 8 || EltVT.getSizeInBits() == 16) &&
        ...
        unsigned Opc;
        if (VT == MVT::v8i16)
            Opc = Cse523ISD::PINSRW;
        else if (VT == MVT::v16i8)
            Opc = Cse523ISD::PINSRB;
        else
            Opc = Cse523ISD::PINSRB;
    ...
    if (EltVT == MVT::f32 && isa<ConstantSDNode>(N2)) {
        ...
        //  the '3' into bits [7:6] of Cse523ISD::INSERTPS.
        ...
        return DAG.getNode(Cse523ISD::INSERTPS, dl, VT, N0, N1, N2);
    }
...
Cse523TargetLowering::LowerINSERT_VECTOR_ELT(SDValue Op, SelectionDAG &DAG) const {
    ...
    if (EltVT.getSizeInBits() == 16 && isa<ConstantSDNode>(N2)) {
        ...
        if (N2.getValueType() != MVT::i32)
            N2 = DAG.getIntPtrConstant(cast<ConstantSDNode>(N2)->getZExtValue());
        return DAG.getNode(Cse523ISD::PINSRW, dl, VT, N0, N1, N2);
    }
    return SDValue();
}
...
static SDValue LowerShiftParts(SDValue Op, SelectionDAG &DAG) {
    ...
    SDValue Cond = DAG.getNode(Cse523ISD::CMP, dl, MVT::i32,
            AndNode, DAG.getConstant(0, MVT::i8));

    SDValue Hi, Lo;
    SDValue CC = DAG.getConstant(Cse523::COND_NE, MVT::i8);
...
SDValue Cse523TargetLowering::EmitTest(SDValue Op, unsigned Cse523CC,
    ...
    if (Op.getValueType() == MVT::i1)
        ...
        return DAG.getNode(Cse523ISD::CMP, dl, MVT::i32, Op,
    ...
    if (Op.getResNo() != 0 || NeedOF || NeedCF) {
        ...
        //if (Op.getValueType() == MVT::i1)
        //  return DAG.getNode(Cse523ISD::CMP, dl, MVT::i1, Op,
...
SDValue Cse523TargetLowering::EmitCmp(SDValue Op0, SDValue Op1, unsigned Cse523CC,
    ...
    return DAG.getNode(Cse523ISD::CMP, dl, MVT::i32, Op0, Op1);
}
...
SDValue Cse523TargetLowering::ConvertCmpIfNecessary(SDValue Cmp,
    ...
    // (Cse523sahf (trunc (srl (Cse523fp_stsw (trunc (Cse523cmp ...)), 8))))
    SDLoc dl(Cmp);
    SDValue TruncFPSW = DAG.getNode(ISD::TRUNCATE, dl, MVT::i16, Cmp);
    SDValue FNStSW = DAG.getNode(Cse523ISD::FNSTSW16r, dl, MVT::i16, TruncFPSW);
    SDValue Srl = DAG.getNode(ISD::SRL, dl, MVT::i16, FNStSW,
            DAG.getConstant(8, MVT::i8));
    SDValue TruncSrl = DAG.getNode(ISD::TRUNCATE, dl, MVT::i8, Srl);
    return DAG.getNode(Cse523ISD::SAHF, dl, MVT::i32, TruncSrl);
}
...
SDValue Cse523TargetLowering::LowerToBT(SDValue And, ISD::CondCode CC,
    ...
    if (LHS.getNode()) {
        ...
        SDValue BT = DAG.getNode(Cse523ISD::BT, dl, MVT::i32, LHS, RHS);
        Cse523::CondCode Cond = CC == ISD::SETEQ ? Cse523::COND_AE : Cse523::COND_B;
        return DAG.getNode(Cse523ISD::SETCC, dl, MVT::i8,
...
static SDValue LowerVSETCC(SDValue Op, const Cse523Subtarget *Subtarget,
        SelectionDAG &DAG) {
    SDValue Op0 = Op.getOperand(0);
    SDValue Op1 = Op.getOperand(1);
    SDValue CC = Op.getOperand(2);
    ...
    if (isFP) {
        ...
        unsigned SSECC = translateCse523FSETCC(SetCCOpcode, Op0, Op1);
        unsigned Opc = Cse523ISD::CMPP;
    ...
    switch (SetCCOpcode) {
        ...
        case ISD::SETEQ:  Opc = Cse523ISD::PCMPEQ; break;
        case ISD::SETLT:  Swap = true;
        case ISD::SETGT:  Opc = Cse523ISD::PCMPGT; break;
        case ISD::SETGE:  Swap = true;
        case ISD::SETLE:  Opc = Cse523ISD::PCMPGT;
                          Invert = true; break;
        case ISD::SETULT: Swap = true;
        case ISD::SETUGT: Opc = Cse523ISD::PCMPGT;
                          FlipSigns = true; break;
        case ISD::SETUGE: Swap = true;
        case ISD::SETULE: Opc = Cse523ISD::PCMPGT;
    ...
    if (hasMinMax) {
        switch (SetCCOpcode) {
            default: break;
            case ISD::SETULE: Opc = Cse523ISD::UMIN; MinMax = true; break;
            case ISD::SETUGE: Opc = Cse523ISD::UMAX; MinMax = true; break;
        }
    ...
    if (VT == MVT::v2i64) {
        if (Opc == Cse523ISD::PCMPGT && !Subtarget->hasSSE42()) {
            ...
            SDValue GT = DAG.getNode(Cse523ISD::PCMPGT, dl, MVT::v4i32, Op0, Op1);
            SDValue EQ = DAG.getNode(Cse523ISD::PCMPEQ, dl, MVT::v4i32, Op0, Op1);
        ...
        if (Opc == Cse523ISD::PCMPEQ && !Subtarget->hasSSE41()) {
    ...
    if (MinMax)
        Result = DAG.getNode(Cse523ISD::PCMPEQ, dl, VT, Op0, Result);

    if (Subus)
        Result = DAG.getNode(Cse523ISD::PCMPEQ, dl, VT, Result,
...
SDValue Cse523TargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
    ...
    if (Op1.getOpcode() == ISD::Constant &&
        ...
        if (Op0.getOpcode() == Cse523ISD::SETCC) {
            ...
            SDValue SetCC = DAG.getNode(Cse523ISD::SETCC, dl, MVT::i8,
    ...
    SDValue SetCC = DAG.getNode(Cse523ISD::SETCC, dl, MVT::i8,
            DAG.getConstant(Cse523CC, MVT::i8), EFLAGS);
    if (VT == MVT::i1)
        return DAG.getNode(ISD::TRUNCATE, dl, MVT::i1, SetCC);
    return SetCC;
}
...
SDValue Cse523TargetLowering::LowerSELECT(SDValue Op, SelectionDAG &DAG) const {
    ...
    if (Cond.getOpcode() == ISD::SETCC &&
        ...
        int SSECC = translateCse523FSETCC(
                cast<CondCodeSDNode>(Cond.getOperand(2))->get(), CondOp0, CondOp1);

        if (SSECC != 8) {
            if (Subtarget->hasAVX512()) {
                SDValue Cmp = DAG.getNode(Cse523ISD::FSETCC, DL, MVT::i1, CondOp0, CondOp1,
                        DAG.getConstant(SSECC, MVT::i8));
                return DAG.getNode(Cse523ISD::SELECT, DL, VT, Cmp, Op1, Op2);
            }
            SDValue Cmp = DAG.getNode(Cse523ISD::FSETCC, DL, VT, CondOp0, CondOp1,
                    DAG.getConstant(SSECC, MVT::i8));
            SDValue AndN = DAG.getNode(Cse523ISD::FANDN, DL, VT, Cmp, Op2);
            SDValue And = DAG.getNode(Cse523ISD::FAND, DL, VT, Cmp, Op1);
            return DAG.getNode(Cse523ISD::FOR, DL, VT, AndN, And);
        }
    }
            ...
            if (ConstantSDNode *YC = dyn_cast<ConstantSDNode>(Y))
                if (YC->isNullValue() &&
                    ...
                    SDValue Res = DAG.getNode(Cse523ISD::SETCC_CARRY, DL, Op.getValueType(),
                            DAG.getConstant(Cse523::COND_B, MVT::i8),
                            SDValue(Neg.getNode(), 1));
                    return Res;
                }

            Cmp = DAG.getNode(Cse523ISD::CMP, DL, MVT::i32,
                    CmpOp0, DAG.getConstant(1, CmpOp0.getValueType()));
            Cmp = ConvertCmpIfNecessary(Cmp, DAG);

            SDValue Res =   // Res = 0 or -1.
                DAG.getNode(Cse523ISD::SETCC_CARRY, DL, Op.getValueType(),
                        DAG.getConstant(Cse523::COND_B, MVT::i8), Cmp);
    ...
    if (CondOpcode == Cse523ISD::SETCC ||
        ...
        CC = DAG.getConstant(Cse523Cond, MVT::i8);
        addTest = false;
    }
    ...
    if (addTest) {
        CC = DAG.getConstant(Cse523::COND_NE, MVT::i8);
    ...
    if (Cond.getOpcode() == Cse523ISD::SUB) {
        ...
        if ((CondCode == Cse523::COND_AE || CondCode == Cse523::COND_B) &&
            ...
            SDValue Res = DAG.getNode(Cse523ISD::SETCC_CARRY, DL, Op.getValueType(),
                    DAG.getConstant(Cse523::COND_B, MVT::i8), Cond);
    ...
    // Cse523 doesn't have an i8 cmov. If both operands are the result of a truncate
    ...
    if (Op.getValueType() == MVT::i8 &&
        ...
        if (T1.getValueType() == T2.getValueType() &&
            ...
            SDValue Cmov = DAG.getNode(Cse523ISD::CMOV, DL, VTs, T2, T1, CC, Cond);
        ...
        if (Cond.hasOneUse() && isAndOrOfSetCCs(Cond, CondOpc)) {
            ...
            if (Op.getNode()->hasOneUse()) {
                ...
                if (User->getOpcode() == ISD::BR) {
                    ...
                    SDValue Cmp = DAG.getNode(Cse523ISD::CMP, dl, MVT::i32,
                            Cond.getOperand(0), Cond.getOperand(1));
                    Cmp = ConvertCmpIfNecessary(Cmp, DAG);
                    CC = DAG.getConstant(Cse523::COND_NE, MVT::i8);
                    Chain = DAG.getNode(Cse523ISD::BRCOND, dl, Op.getValueType(),
                            Chain, Dest, CC, Cmp);
                    CC = DAG.getConstant(Cse523::COND_P, MVT::i8);
                    Cond = Cmp;
                    addTest = false;
                }
            }
            ...
            if (Op.getNode()->hasOneUse()) {
                ...
                if (User->getOpcode() == ISD::BR) {
                    ...
                    Chain = DAG.getNode(Cse523ISD::BRCOND, dl, Op.getValueType(),
                            Chain, Dest, CC, Cmp);
                    CC = DAG.getConstant(Cse523::COND_NP, MVT::i8);
                    Cond = Cmp;
                    addTest = false;
                    Dest = FalseBB;
                }
...
static SDValue LowerXALUO(SDValue Op, SelectionDAG &DAG) {
    ...
    switch (Op.getOpcode()) {
        ...
        case ISD::UMULO: { // i64, i8 = umulo lhs, rhs --> i64, i64, i32 umul lhs,rhs
                             ...
                             SDValue SetCC =
                                 DAG.getNode(Cse523ISD::SETCC, DL, MVT::i8,
                                         DAG.getConstant(Cse523::COND_O, MVT::i32),
...
SDValue Cse523TargetLowering::LowerSIGN_EXTEND_INREG(SDValue Op,
    ...
    switch (VT.SimpleTy) {
        ...
        case MVT::v8i16: {
                             ...
                             if (Op0.getOpcode() == ISD::BITCAST &&
                                 ...
                                 if (Tmp1.getNode()) {
                                     ...
                                     if (ExtraEltVT == MVT::i8 || ExtraEltVT == MVT::i16 ||
                                         ...
                                         assert(Tmp1Op0.getOpcode() == Cse523ISD::VZEXT &&
                                                 "This optimization is invalid without a VZEXT.");
                                         return DAG.getNode(Cse523ISD::VSEXT, dl, VT, Tmp1Op0.getOperand(0));
                                     }
                                     Op0 = Tmp1;
                                 }
                             }
                             ...
                             Tmp1 = getTargetVShiftByConstNode(Cse523ISD::VSHLI, dl, VT, Op0, BitsDiff,
                                     DAG);
                             return getTargetVShiftByConstNode(Cse523ISD::VSRAI, dl, VT, Tmp1, BitsDiff,
                                     DAG);
                         }
    }
}
...
static SDValue LowerATOMIC_FENCE(SDValue Op, const Cse523Subtarget *Subtarget,
    ...
    if (FenceOrdering == SequentiallyConsistent && FenceScope == CrossThread) {
        ...
        SDValue Ops[] = {
            DAG.getRegister(Cse523::RSP, MVT::i32), // Base //TODO: Replaced
        ...
        assert(0);
        //SDNode *Res = DAG.getMachineNode(Cse523::OR32mrLocked, dl, MVT::Other, Ops);
...
Cse523TargetLowering::emitFMA3Instr(MachineInstr *MI,
    ...
    //        switch (MI->getOpcode()) {
    //            case Cse523::VFMADDPDr213r: NewFMAOpc = Cse523::VFMADDPDr231r; break;
    //            case Cse523::VFMADDPSr213r: NewFMAOpc = Cse523::VFMADDPSr231r; break;
    //            case Cse523::VFMADDSDr213r: NewFMAOpc = Cse523::VFMADDSDr231r; break;
    //            case Cse523::VFMADDSSr213r: NewFMAOpc = Cse523::VFMADDSSr231r; break;
    //            case Cse523::VFMSUBPDr213r: NewFMAOpc = Cse523::VFMSUBPDr231r; break;
    //            case Cse523::VFMSUBPSr213r: NewFMAOpc = Cse523::VFMSUBPSr231r; break;
    //            case Cse523::VFMSUBSDr213r: NewFMAOpc = Cse523::VFMSUBSDr231r; break;
    //            case Cse523::VFMSUBSSr213r: NewFMAOpc = Cse523::VFMSUBSSr231r; break;
    //            case Cse523::VFNMADDPDr213r: NewFMAOpc = Cse523::VFNMADDPDr231r; break;
    //            case Cse523::VFNMADDPSr213r: NewFMAOpc = Cse523::VFNMADDPSr231r; break;
    //            case Cse523::VFNMADDSDr213r: NewFMAOpc = Cse523::VFNMADDSDr231r; break;
    //            case Cse523::VFNMADDSSr213r: NewFMAOpc = Cse523::VFNMADDSSr231r; break;
    //            case Cse523::VFNMSUBPDr213r: NewFMAOpc = Cse523::VFNMSUBPDr231r; break;
    //            case Cse523::VFNMSUBPSr213r: NewFMAOpc = Cse523::VFNMSUBPSr231r; break;
    //            case Cse523::VFNMSUBSDr213r: NewFMAOpc = Cse523::VFNMSUBSDr231r; break;
    //            case Cse523::VFNMSUBSSr213r: NewFMAOpc = Cse523::VFNMSUBSSr231r; break;
    //            case Cse523::VFMADDPDr213rY: NewFMAOpc = Cse523::VFMADDPDr231rY; break;
    //            case Cse523::VFMADDPSr213rY: NewFMAOpc = Cse523::VFMADDPSr231rY; break;
    //            case Cse523::VFMSUBPDr213rY: NewFMAOpc = Cse523::VFMSUBPDr231rY; break;
    //            case Cse523::VFMSUBPSr213rY: NewFMAOpc = Cse523::VFMSUBPSr231rY; break;
    //            case Cse523::VFNMADDPDr213rY: NewFMAOpc = Cse523::VFNMADDPDr231rY; break;
    //            case Cse523::VFNMADDPSr213rY: NewFMAOpc = Cse523::VFNMADDPSr231rY; break;
    //            case Cse523::VFNMSUBPDr213rY: NewFMAOpc = Cse523::VFNMSUBPDr231rY; break;
    //            case Cse523::VFNMSUBPSr213rY: NewFMAOpc = Cse523::VFNMSUBPSr231rY; break;
...
//                 return BB;
//        case Cse523::WIN_ALLOCA:
//                 return EmitLoweredWinAlloca(MI, BB);
//        case Cse523::SEG_ALLOCA_32:
//                 return EmitLoweredSegAlloca(MI, BB, false);
//        case Cse523::SEG_ALLOCA_64:
//                 return EmitLoweredSegAlloca(MI, BB, true);
//        case Cse523::TLSCall_32:
//        case Cse523::TLSCall_64:
//                 return EmitLoweredTLSCall(MI, BB);
//        case Cse523::CMOV_GR8:
//        case Cse523::CMOV_FR32:
//        case Cse523::CMOV_FR64:
//        case Cse523::CMOV_V4F32:
//        case Cse523::CMOV_V2F64:
//        case Cse523::CMOV_V2I64:
//        case Cse523::CMOV_V8F32:
//        case Cse523::CMOV_V4F64:
//        case Cse523::CMOV_V4I64:
//        case Cse523::CMOV_V16F32:
//        case Cse523::CMOV_V8F64:
//        case Cse523::CMOV_V8I64:
//        case Cse523::CMOV_GR16:
//        case Cse523::CMOV_GR32:
//        case Cse523::CMOV_RFP32:
//        case Cse523::CMOV_RFP64:
//        case Cse523::CMOV_RFP80:
//                 return EmitLoweredSelect(MI, BB);
//
//        case Cse523::FP32_TO_INT16_IN_MEM:
//        case Cse523::FP32_TO_INT32_IN_MEM:
//        case Cse523::FP32_TO_INT64_IN_MEM:
//        case Cse523::FP64_TO_INT16_IN_MEM:
//        case Cse523::FP64_TO_INT32_IN_MEM:
//        case Cse523::FP64_TO_INT64_IN_MEM:
//        case Cse523::FP80_TO_INT16_IN_MEM:
//        case Cse523::FP80_TO_INT32_IN_MEM:
//        case Cse523::FP80_TO_INT64_IN_MEM: {
//                                               const TargetInstrInfo *TII = getTargetMachine().getInstrInfo();
//                                               DebugLoc DL = MI->getDebugLoc();
//
...
//                                                           TII->get(Cse523::FNSTCW16m)), CWFrameIdx);
//
//                                               // Load the old value of the high byte of the control word...
//                                               unsigned OldCW =
//                                                   F->getRegInfo().createVirtualRegister(&Cse523::GR16RegClass);
//                                               addFrameReference(BuildMI(*BB, MI, DL, TII->get(Cse523::MOV16rm), OldCW),
//                                                       CWFrameIdx);
//
//                                               // Set the high part to be round to zero...
//                                               addFrameReference(BuildMI(*BB, MI, DL, TII->get(Cse523::MOV16mi)), CWFrameIdx)
//                                                   .addImm(0xC7F);
//
//                                               // Reload the modified control word now...
//                                               addFrameReference(BuildMI(*BB, MI, DL,
//                                                           TII->get(Cse523::FLDCW16m)), CWFrameIdx);
//
//                                               // Restore the memory image of control word to original value
//                                               addFrameReference(BuildMI(*BB, MI, DL, TII->get(Cse523::MOV16mr)), CWFrameIdx)
//                                                   .addReg(OldCW);
//
//                                               // Get the Cse523 opcode to use.
//                                               unsigned Opc;
//                                               switch (MI->getOpcode()) {
//                                                   default: llvm_unreachable("illegal opcode!");
//                                                   case Cse523::FP32_TO_INT16_IN_MEM: Opc = Cse523::IST_Fp16m32; break;
//                                                   case Cse523::FP32_TO_INT32_IN_MEM: Opc = Cse523::IST_Fp32m32; break;
//                                                   case Cse523::FP32_TO_INT64_IN_MEM: Opc = Cse523::IST_Fp64m32; break;
//                                                   case Cse523::FP64_TO_INT16_IN_MEM: Opc = Cse523::IST_Fp16m64; break;
//                                                   case Cse523::FP64_TO_INT32_IN_MEM: Opc = Cse523::IST_Fp32m64; break;
//                                                   case Cse523::FP64_TO_INT64_IN_MEM: Opc = Cse523::IST_Fp64m64; break;
//                                                   case Cse523::FP80_TO_INT16_IN_MEM: Opc = Cse523::IST_Fp16m80; break;
//                                                   case Cse523::FP80_TO_INT32_IN_MEM: Opc = Cse523::IST_Fp32m80; break;
//                                                   case Cse523::FP80_TO_INT64_IN_MEM: Opc = Cse523::IST_Fp64m80; break;
//                                               }
//
//                                               Cse523AddressMode AM;
//                                               MachineOperand &Op = MI->getOperand(0);
//                                               if (Op.isReg()) {
//                                                   AM.BaseType = Cse523AddressMode::RegBase;
...
//                                                   .addReg(MI->getOperand(Cse523::AddrNumOperands).getReg());
//
...
//        case Cse523::PCMPISTRM128REG:
//        case Cse523::VPCMPISTRM128REG:
//        case Cse523::PCMPISTRM128MEM:
//        case Cse523::VPCMPISTRM128MEM:
//        case Cse523::PCMPESTRM128REG:
//        case Cse523::VPCMPESTRM128REG:
//        case Cse523::PCMPESTRM128MEM:
//        case Cse523::VPCMPESTRM128MEM:
//                                           assert(Subtarget->hasSSE42() &&
//                                                   "Target must have SSE4.2 or AVX features enabled");
//                                           return EmitPCMPSTRM(MI, BB, getTargetMachine().getInstrInfo());
//
//                                           // String/text processing lowering.
//        case Cse523::PCMPISTRIREG:
//        case Cse523::VPCMPISTRIREG:
//        case Cse523::PCMPISTRIMEM:
//        case Cse523::VPCMPISTRIMEM:
//        case Cse523::PCMPESTRIREG:
//        case Cse523::VPCMPESTRIREG:
//        case Cse523::PCMPESTRIMEM:
//        case Cse523::VPCMPESTRIMEM:
//                                           assert(Subtarget->hasSSE42() &&
//                                                   "Target must have SSE4.2 or AVX features enabled");
...
//
//        case Cse523::VASTART_SAVE_XMM_REGS:
//                                           return EmitVAStartSaveXMMRegsWithCustomInserter(MI, BB);
//
//        case Cse523::VAARG_64:
//                                           return EmitVAARG64WithCustomInserter(MI, BB);
//
//        case Cse523::EH_SjLj_SetJmp32:
//        case Cse523::EH_SjLj_SetJmp64:
//                                           return emitEHSjLjSetJmp(MI, BB);
//
//        case Cse523::EH_SjLj_LongJmp32:
//        case Cse523::EH_SjLj_LongJmp64:
...
//
//        case Cse523::VFMADDPDr213r:
//        case Cse523::VFMADDPSr213r:
//        case Cse523::VFMADDSDr213r:
//        case Cse523::VFMADDSSr213r:
//        case Cse523::VFMSUBPDr213r:
//        case Cse523::VFMSUBPSr213r:
//        case Cse523::VFMSUBSDr213r:
//        case Cse523::VFMSUBSSr213r:
//        case Cse523::VFNMADDPDr213r:
//        case Cse523::VFNMADDPSr213r:
//        case Cse523::VFNMADDSDr213r:
//        case Cse523::VFNMADDSSr213r:
//        case Cse523::VFNMSUBPDr213r:
//        case Cse523::VFNMSUBPSr213r:
//        case Cse523::VFNMSUBSDr213r:
//        case Cse523::VFNMSUBSSr213r:
//        case Cse523::VFMADDPDr213rY:
//        case Cse523::VFMADDPSr213rY:
//        case Cse523::VFMSUBPDr213rY:
//        case Cse523::VFMSUBPSr213rY:
//        case Cse523::VFNMADDPDr213rY:
//        case Cse523::VFNMADDPSr213rY:
//        case Cse523::VFNMSUBPDr213rY:
//        case Cse523::VFNMSUBPSr213rY:
...
matchIntegerMINMAX(SDValue Cond, EVT VT, SDValue LHS, SDValue RHS,
    ...
    if (DAG.isEqualTo(LHS, Cond.getOperand(0)) &&
        ...
        switch (CC) {
            ...
            case ISD::SETULT:
            case ISD::SETULE:
                     Opc = hasUnsigned ? Cse523ISD::UMIN : 0; break;
            case ISD::SETUGT:
            case ISD::SETUGE:
                     Opc = hasUnsigned ? Cse523ISD::UMAX : 0; break;
            case ISD::SETLT:
            case ISD::SETLE:
                     Opc = hasSigned ? Cse523ISD::SMIN : 0; break;
            case ISD::SETGT:
            case ISD::SETGE:
                     Opc = hasSigned ? Cse523ISD::SMAX : 0; break;
        }
    ...
    if (ConstantSDNode *TrueC = dyn_cast<ConstantSDNode>(TrueOp)) {
        if (ConstantSDNode *FalseC = dyn_cast<ConstantSDNode>(FalseOp)) {
            ...
            if (FalseC->getAPIntValue() == 0 && TrueC->getAPIntValue().isPowerOf2()) {
                Cond = DAG.getNode(Cse523ISD::SETCC, DL, MVT::i8,
        ...
        if (VT == MVT::f32 || VT == MVT::f64) {
            ...
            if (!ExpectingFlags) {
                ...
                if ((cc0 == Cse523::COND_E  && cc1 == Cse523::COND_NP) ||
                    ...
                    if (Subtarget->hasAVX512()) {
                        SDValue FSetCC = DAG.getNode(Cse523ISD::FSETCC, DL, MVT::i1, CMP00,
                                CMP01, DAG.getConstant(cse523cc, MVT::i8));
    ...
    if (VT.isInteger() && N->getOpcode() == ISD::XOR &&
        ...
        if (ConstantSDNode *Y1C = dyn_cast<ConstantSDNode>(N1.getOperand(1)))
            if (Y1C->getAPIntValue() == VT.getSizeInBits()-1) {
                // Generate SUB & CMOV.
                SDValue Neg = DAG.getNode(Cse523ISD::SUB, DL, DAG.getVTList(VT, MVT::i32),
                        DAG.getConstant(0, VT), N0.getOperand(0));

                SDValue Ops[] = { N0.getOperand(0), Neg,
                    DAG.getConstant(Cse523::COND_GE, MVT::i8),
                    SDValue(Neg.getNode(), 1) };
...
static SDValue MaterializeSETB(SDLoc DL, SDValue EFLAGS, SelectionDAG &DAG,
    ...
    if (VT == MVT::i8)
        return DAG.getNode(ISD::AND, DL, VT,
                DAG.getNode(Cse523ISD::SETCC_CARRY, DL, MVT::i8,
                    DAG.getConstant(Cse523::COND_B, MVT::i8), EFLAGS),
                DAG.getConstant(1, VT));
...
static SDValue OptimizeConditionalInDecrement(SDNode *N, SelectionDAG &DAG) {
    ...
    SDValue CmpOp0 = Cmp.getOperand(0);
    SDValue NewCmp = DAG.getNode(Cse523ISD::CMP, DL, MVT::i32, CmpOp0,
