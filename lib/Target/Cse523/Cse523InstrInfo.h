//===-- Cse523InstrInfo.h - Cse523 Instruction Information ------------*- C++ -*-===//
//
...
#ifndef CSE523INSTRUCTIONINFO_H
#define CSE523INSTRUCTIONINFO_H
...
namespace llvm {
  class Cse523RegisterInfo;
  class Cse523TargetMachine;

namespace Cse523 {
  // Cse523 specific condition code. These correspond to Cse523_*_COND in
  // Cse523InstrInfo.td. They must be kept in synch.
  enum CondCode {
    COND_A  = 0,
    COND_AE = 1,
    COND_B  = 2,
    ...
    // which can't be represented on cse523 with a single condition. These
  ...
  CondCode GetOppositeBranchCondition(Cse523::CondCode CC);
...
inline static bool isGlobalStubReference(unsigned char TargetFlag) {
  switch (TargetFlag) {
  case Cse523II::MO_DLLIMPORT: // dllimport stub.
  case Cse523II::MO_GOTPCREL:  // rip-relative GOT reference.
  case Cse523II::MO_GOT:       // normal GOT reference.
  case Cse523II::MO_DARWIN_NONLAZY_PIC_BASE:        // Normal $non_lazy_ptr ref.
  case Cse523II::MO_DARWIN_NONLAZY:                 // Normal $non_lazy_ptr ref.
  case Cse523II::MO_DARWIN_HIDDEN_NONLAZY_PIC_BASE: // Hidden $non_lazy_ptr ref.
    return true;
  default:
    return false;
  }
...
/// reference is relative to a 32-bit PIC base (Cse523ISD::GlobalBaseReg).  If this
/// is true, the addressing mode has the PIC base register added in (e.g. EBX).
inline static bool isGlobalRelativeToPICBase(unsigned char TargetFlag) {
  switch (TargetFlag) {
  case Cse523II::MO_GOTOFF:                         // isPICStyleGOT: local global.
  case Cse523II::MO_GOT:                            // isPICStyleGOT: other global.
  case Cse523II::MO_PIC_BASE_OFFSET:                // Darwin local global.
  case Cse523II::MO_DARWIN_NONLAZY_PIC_BASE:        // Darwin/32 external global.
  case Cse523II::MO_DARWIN_HIDDEN_NONLAZY_PIC_BASE: // Darwin/32 hidden global.
  case Cse523II::MO_TLVP:                           // ??? Pretty sure..
    return true;
  default:
    return false;
  }
...
class Cse523InstrInfo : public Cse523GenInstrInfo {
  Cse523TargetMachine &TM;
  const Cse523RegisterInfo RI;
...
public:
  explicit Cse523InstrInfo(Cse523TargetMachine &tm);

  /// getRegisterInfo - TargetInstrInfo is a superset of MRegister info.  As
  /// such, whenever a client has an instance of instruction info, it should
  /// always be able to get register info as well (through this method).
  ///
  virtual const Cse523RegisterInfo &getRegisterInfo() const { return RI; }

  /// isCoalescableExtInstr - Return true if the instruction is a "coalescable"
  /// extension instruction. That is, it's like a copy where it's legal for the
  /// source to overlap the destination. e.g. Cse523::MOVSX64rr32. If this returns
...
//                                       unsigned NumLoads) const;
  ...
  static bool isCse523_64ExtendedReg(const MachineOperand &MO) {
    return false;
  }
