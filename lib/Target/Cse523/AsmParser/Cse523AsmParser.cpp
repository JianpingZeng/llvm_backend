...
namespace {
    ...
    class Cse523AsmParser : public MCTargetAsmParser {
        ...
        Cse523Operand *ErrorOperand(SMLoc Loc, StringRef Msg) {
            Error(Loc, Msg);
            return 0;
        }

        Cse523Operand *DefaultMemSIOperand(SMLoc Loc);
        Cse523Operand *DefaultMemDIOperand(SMLoc Loc);
        Cse523Operand *ParseOperand();
        Cse523Operand *ParseATTOperand();
        Cse523Operand *ParseIntelOperand();
        Cse523Operand *ParseIntelOffsetOfOperator();
        bool ParseIntelDotOperator(const MCExpr *Disp, const MCExpr *&NewDisp);
        Cse523Operand *ParseIntelOperator(unsigned OpKind);
        Cse523Operand *ParseIntelSegmentOverride(unsigned SegReg, SMLoc Start, unsigned Size);
        Cse523Operand *ParseIntelMemOperand(int64_t ImmDisp, SMLoc StartLoc,
                unsigned Size);
        bool ParseIntelExpression(IntelExprStateMachine &SM, SMLoc &End);
        Cse523Operand *ParseIntelBracExpression(unsigned SegReg, SMLoc Start,
                int64_t ImmDisp, unsigned Size);
        bool ParseIntelIdentifier(const MCExpr *&Val, StringRef &Identifier,
                InlineAsmIdentifierInfo &Info,
                bool IsUnevaluatedOperand, SMLoc &End);

        Cse523Operand *ParseMemOperand(unsigned SegReg, SMLoc StartLoc);

        Cse523Operand *CreateMemForInlineAsm(unsigned SegReg, const MCExpr *Disp,
        ...
        bool doSrcDstMatch(Cse523Operand &Op1, Cse523Operand &Op2);
        ...
        bool is64BitMode() const {
            ...
            return true;
            //return (STI.getFeatureBits() & Cse523::Mode64Bit) != 0;
        }
        bool is32BitMode() const {
            // FIXME: Can tablegen auto-generate this?
            return false;
            //return (STI.getFeatureBits() & Cse523::Mode32Bit) != 0;
        }
        bool is16BitMode() const {
            // FIXME: Can tablegen auto-generate this?
            return false;
            //return (STI.getFeatureBits() & Cse523::Mode16Bit) != 0;
        }
        void SwitchMode(uint64_t mode) {
            return;
//            uint64_t oldMode = STI.getFeatureBits() &
//                (Cse523::Mode64Bit | Cse523::Mode32Bit | Cse523::Mode16Bit);
...
namespace {
    ...
    /// instruction.
    struct Cse523Operand : public MCParsedAsmOperand {
        enum KindTy {
            Token,
            Register,
            Immediate,
        ...
        Cse523Operand(KindTy K, SMLoc Start, SMLoc End)
        ...
        bool isSrcIdx() const {
            return !getMemIndexReg() && getMemScale() == 1 &&
              (getMemBaseReg() == Cse523::RSI) && isa<MCConstantExpr>(getMemDisp()) &&
        ...
        bool isDstIdx() const {
            return !getMemIndexReg() && getMemScale() == 1 &&
              (getMemSegReg() == 0 || getMemSegReg() == Cse523::ES) &&
              (getMemBaseReg() == Cse523::RDI) && isa<MCConstantExpr>(getMemDisp()) &&
              cast<MCConstantExpr>(getMemDisp())->getValue() == 0;
        }
        ...
        void addGR32orGR64Operands(MCInst &Inst, unsigned N) const {
            llvm_unreachable("Unexpected path addGR32orGR64Operands()");
        }
        ...
        static Cse523Operand *CreateToken(StringRef Str, SMLoc Loc) {
            SMLoc EndLoc = SMLoc::getFromPointer(Loc.getPointer() + Str.size());
            Cse523Operand *Res = new Cse523Operand(Token, Loc, EndLoc);
            Res->Tok.Data = Str.data();
            Res->Tok.Length = Str.size();
            return Res;
        }

        static Cse523Operand *CreateReg(unsigned RegNo, SMLoc StartLoc, SMLoc EndLoc,
                bool AddressOf = false,
                SMLoc OffsetOfLoc = SMLoc(),
                StringRef SymName = StringRef(),
                void *OpDecl = 0) {
            Cse523Operand *Res = new Cse523Operand(Register, StartLoc, EndLoc);
            Res->Reg.RegNo = RegNo;
        ...
        static Cse523Operand *CreateImm(const MCExpr *Val, SMLoc StartLoc, SMLoc EndLoc){
            Cse523Operand *Res = new Cse523Operand(Immediate, StartLoc, EndLoc);
            Res->Imm.Val = Val;
            return Res;
        }

        /// Create an absolute memory operand.
        static Cse523Operand *CreateMem(const MCExpr *Disp, SMLoc StartLoc, SMLoc EndLoc,
                unsigned Size = 0, StringRef SymName = StringRef(),
                void *OpDecl = 0) {
            Cse523Operand *Res = new Cse523Operand(Memory, StartLoc, EndLoc);
            Res->Mem.SegReg   = 0;
            Res->Mem.Disp     = Disp;
            Res->Mem.BaseReg  = 0;
            Res->Mem.IndexReg = 0;
        ...
        static Cse523Operand *CreateMem(unsigned SegReg, const MCExpr *Disp,
                unsigned BaseReg, unsigned IndexReg,
...
//        if (Cse523MCRegisterClasses[Cse523::GR64RegClassID].contains(BaseReg) &&
//                (Cse523MCRegisterClasses[Cse523::GR16RegClassID].contains(IndexReg) ||
//                 Cse523MCRegisterClasses[Cse523::GR32RegClassID].contains(IndexReg)) &&
//                IndexReg != Cse523::RIZ) {
//            ErrMsg = "base register is 64-bit, but index register is not";
//            return true;
//        }
//        if (Cse523MCRegisterClasses[Cse523::GR32RegClassID].contains(BaseReg) &&
//                (Cse523MCRegisterClasses[Cse523::GR16RegClassID].contains(IndexReg) ||
//                 Cse523MCRegisterClasses[Cse523::GR64RegClassID].contains(IndexReg)) &&
//                IndexReg != Cse523::EIZ){
...
//            if (Cse523MCRegisterClasses[Cse523::GR32RegClassID].contains(IndexReg) ||
//                    Cse523MCRegisterClasses[Cse523::GR64RegClassID].contains(IndexReg)) {
//                ErrMsg = "base register is 16-bit, but index register is not";
//                return true;
//            }
//            if (((BaseReg == Cse523::BX || BaseReg == Cse523::BP) &&
//                        IndexReg != Cse523::SI && IndexReg != Cse523::DI) ||
//                    ((BaseReg == Cse523::SI || BaseReg == Cse523::DI) &&
//                     IndexReg != Cse523::BX && IndexReg != Cse523::BP)) {
...
bool Cse523AsmParser::doSrcDstMatch(Cse523Operand &Op1, Cse523Operand &Op2)
{
...
bool Cse523AsmParser::ParseRegister(unsigned &RegNo,
...
//        Parser.Lex(); // Eat ')'
    ...
    if (RegNo == 0 && Tok.getString().size() == 3 &&
        ...
        switch (Tok.getString()[2]) {
            case '0': RegNo = Cse523::DR0; break;
            case '1': RegNo = Cse523::DR1; break;
            case '2': RegNo = Cse523::DR2; break;
            case '3': RegNo = Cse523::DR3; break;
            case '4': RegNo = Cse523::DR4; break;
            case '5': RegNo = Cse523::DR5; break;
            case '6': RegNo = Cse523::DR6; break;
            case '7': RegNo = Cse523::DR7; break;
        }
...
Cse523Operand *Cse523AsmParser::DefaultMemSIOperand(SMLoc Loc) {
    unsigned basereg = Cse523::RSI;
    const MCExpr *Disp = MCConstantExpr::Create(0, getContext());
    return Cse523Operand::CreateMem(/*SegReg=*/0, Disp, /*BaseReg=*/basereg,
            /*IndexReg=*/0, /*Scale=*/1, Loc, Loc, 0);
}

Cse523Operand *Cse523AsmParser::DefaultMemDIOperand(SMLoc Loc) {
    unsigned basereg = Cse523::RDI;
    const MCExpr *Disp = MCConstantExpr::Create(0, getContext());
    return Cse523Operand::CreateMem(/*SegReg=*/0, Disp, /*BaseReg=*/basereg,
            /*IndexReg=*/0, /*Scale=*/1, Loc, Loc, 0);
}

Cse523Operand *Cse523AsmParser::ParseOperand() {
...
Cse523Operand *
Cse523AsmParser::CreateMemForInlineAsm(unsigned SegReg, const MCExpr *Disp,
        unsigned BaseReg, unsigned IndexReg,
        unsigned Scale, SMLoc Start, SMLoc End,
        unsigned Size, StringRef Identifier,
        InlineAsmIdentifierInfo &Info){
    ...
    return Cse523Operand::CreateMem(SegReg, Disp, BaseReg, IndexReg, Scale, Start,
...
bool Cse523AsmParser::ParseIntelExpression(IntelExprStateMachine &SM, SMLoc &End) {
    const AsmToken &Tok = Parser.getTok();
...
Cse523Operand *Cse523AsmParser::ParseIntelBracExpression(unsigned SegReg, SMLoc Start,
        int64_t ImmDisp,
        unsigned Size) {
    const AsmToken &Tok = Parser.getTok();
    ...
    if (!isParsingInlineAsm()) {
        ...
        if (!BaseReg && !IndexReg) {
            if (!SegReg)
                return Cse523Operand::CreateMem(Disp, Start, End, Size);
            else
                return Cse523Operand::CreateMem(SegReg, Disp, 0, 0, 1, Start, End, Size);
        }
        StringRef ErrMsg;
...
bool Cse523AsmParser::ParseIntelIdentifier(const MCExpr *&Val,
...
Cse523Operand *Cse523AsmParser::ParseIntelSegmentOverride(unsigned SegReg,
        SMLoc Start,
        unsigned Size) {
    ...
    if (getLexer().is(AsmToken::Integer)) {
        ...
        if (getLexer().isNot(AsmToken::LBrac)) {
            ...
            return Cse523Operand::CreateMem(SegReg, Disp, /*BaseReg=*/0, /*IndexReg=*/0,
    ...
    if (!isParsingInlineAsm()) {
        ...
        return Cse523Operand::CreateMem(Val, Start, End, Size);
    }
...
Cse523Operand *Cse523AsmParser::ParseIntelMemOperand(int64_t ImmDisp, SMLoc Start,
        unsigned Size) {
    const AsmToken &Tok = Parser.getTok();
    SMLoc End;
...
/// Parse the '.' operator.
bool Cse523AsmParser::ParseIntelDotOperator(const MCExpr *Disp,
        const MCExpr *&NewDisp) {
    const AsmToken &Tok = Parser.getTok();
...
Cse523Operand *Cse523AsmParser::ParseIntelOffsetOfOperator() {
    const AsmToken &Tok = Parser.getTok();
...
Cse523Operand *Cse523AsmParser::ParseIntelOperator(unsigned OpKind) {
    const AsmToken &Tok = Parser.getTok();
    ...
    return Cse523Operand::CreateImm(Imm, Start, End);
}

Cse523Operand *Cse523AsmParser::ParseIntelOperand() {
    const AsmToken &Tok = Parser.getTok();
    SMLoc Start, End;
    ...
    if (getLexer().is(AsmToken::Integer) || getLexer().is(AsmToken::Minus) ||
        ...
        if (getLexer().isNot(AsmToken::LBrac)) {
            ...
            if (SM.getSym())
                return Cse523Operand::CreateMem(SM.getSym(), Start, End, Size);

            const MCExpr *ImmExpr = MCConstantExpr::Create(Imm, getContext());
            return Cse523Operand::CreateImm(ImmExpr, Start, End);
        }
    ...
    if (!ParseRegister(RegNo, Start, End)) {
        ...
        if (getLexer().isNot(AsmToken::Colon))
            return Cse523Operand::CreateReg(RegNo, Start, End);
...
Cse523Operand *Cse523AsmParser::ParseATTOperand() {
    switch (getLexer().getKind()) {
        default:
        ...
        case AsmToken::Dollar: {
                                   ...
                                   if (getParser().parseExpression(Val, End))
                                       return 0;
                                   return Cse523Operand::CreateImm(Val, Start, End);
                               }
    }
}

bool
Cse523AsmParser::HandleAVX512Operand(SmallVectorImpl<MCParsedAsmOperand*> &Operands,
        const MCParsedAsmOperand &Op) {
    return false;
}
...
Cse523Operand *Cse523AsmParser::ParseMemOperand(unsigned SegReg, SMLoc MemStart) {
    ...
    if (getLexer().isNot(AsmToken::LParen)) {
        ...
        if (getLexer().isNot(AsmToken::LParen)) {
            ...
            if (SegReg == 0)
                return Cse523Operand::CreateMem(Disp, MemStart, ExprEnd);
            return Cse523Operand::CreateMem(SegReg, Disp, 0, 0, 1, MemStart, ExprEnd);
        }
        ...
        if (getLexer().is(AsmToken::Percent) || getLexer().is(AsmToken::Comma)) {
            ...
            if (getLexer().isNot(AsmToken::LParen)) {
                // Unless we have a segment register, treat this as an immediate.
                if (SegReg == 0)
                    return Cse523Operand::CreateMem(Disp, LParenLoc, ExprEnd);
                return Cse523Operand::CreateMem(SegReg, Disp, 0, 0, 1, MemStart, ExprEnd);
            }
...
//    }
    ...
    return Cse523Operand::CreateMem(SegReg, Disp, BaseReg, IndexReg, Scale,
            MemStart, MemEnd);
}
...
ParseInstruction(ParseInstructionInfo &Info, StringRef Name, SMLoc NameLoc,
    ...
    Operands.push_back(Cse523Operand::CreateToken(PatchedName, NameLoc));

    if (ExtraImmOp && !isParsingIntelSyntax())
        Operands.push_back(Cse523Operand::CreateImm(ExtraImmOp, NameLoc, NameLoc));
    ...
    if (getLexer().isNot(AsmToken::EndOfStatement) && !isPrefix) {
        ...
        if (getLexer().is(AsmToken::Star))
            Operands.push_back(Cse523Operand::CreateToken("*", consumeToken()));

        // Read the operands.
        while(1) {
            if (Cse523Operand *Op = ParseOperand()) {
    ...
    if ((Name == "outb" || Name == "outw" || Name == "outl" || Name == "out") &&
            Operands.size() == 3) {
        Cse523Operand &Op = *(Cse523Operand*)Operands.back();
        if (Op.isMem() && Op.Mem.SegReg == 0 &&
                isa<MCConstantExpr>(Op.Mem.Disp) &&
                cast<MCConstantExpr>(Op.Mem.Disp)->getValue() == 0 &&
                Op.Mem.BaseReg == MatchRegisterName("dx") && Op.Mem.IndexReg == 0) {
            SMLoc Loc = Op.getEndLoc();
            Operands.back() = Cse523Operand::CreateReg(Op.Mem.BaseReg, Loc, Loc);
            delete &Op;
        }
    }
    ...
    if ((Name == "inb" || Name == "inw" || Name == "inl" || Name == "in") &&
            Operands.size() == 3) {
        Cse523Operand &Op = *(Cse523Operand*)Operands.begin()[1];
        if (Op.isMem() && Op.Mem.SegReg == 0 &&
                isa<MCConstantExpr>(Op.Mem.Disp) &&
                cast<MCConstantExpr>(Op.Mem.Disp)->getValue() == 0 &&
                Op.Mem.BaseReg == MatchRegisterName("dx") && Op.Mem.IndexReg == 0) {
            SMLoc Loc = Op.getEndLoc();
            Operands.begin()[1] = Cse523Operand::CreateReg(Op.Mem.BaseReg, Loc, Loc);
            delete &Op;
        }
    }
...
//        }
    ...
    if (Name.startswith("cmps") &&
        ...
        if (Operands.size() == 1) {
            ...
            Cse523Operand &Op = *(Cse523Operand*)Operands.begin()[1];
            Cse523Operand &Op2 = *(Cse523Operand*)Operands.begin()[2];
    ...
    if ((Name.startswith("movs") &&
        ...
        if (Operands.size() == 1) {
            if (Name == "movsd")
                Operands.back() = Cse523Operand::CreateToken("movsl", NameLoc);
            if (isParsingIntelSyntax()) {
                Operands.push_back(DefaultMemDIOperand(NameLoc));
                Operands.push_back(DefaultMemSIOperand(NameLoc));
            } else {
    ...
    if ((Name.startswith("shr") || Name.startswith("sar") ||
        ...
        if (isParsingIntelSyntax()) {
            // Intel syntax
            Cse523Operand *Op1 = static_cast<Cse523Operand*>(Operands[2]);
            ...
            Cse523Operand *Op1 = static_cast<Cse523Operand*>(Operands[1]);
            if (Op1->isImm() && isa<MCConstantExpr>(Op1->getImm()) &&
                    cast<MCConstantExpr>(Op1->getImm())->getValue() == 1) {
                delete Operands[1];
    ...
    if (Name == "int" && Operands.size() == 2) {
        ...
        if (Op1->isImm() && isa<MCConstantExpr>(Op1->getImm()) &&
            ...
            delete Operands[1];
            Operands.erase(Operands.begin() + 1);
            static_cast<Cse523Operand*>(Operands[0])->setTokenValue("int3");
        }
    }
...
static bool convert64i32to64ri8(MCInst &Inst, unsigned Opcode,
    ...
    return convertToSExti8(Inst, Opcode, Cse523::RAX, isCmp);
}
#endif
...
processInstruction(MCInst &Inst,
    ...
    switch (Inst.getOpcode()) {
        default: return false;
                 //case Cse523::AND16i16: return convert16i16to16ri8(Inst, Cse523::AND16ri8);
                 //case Cse523::AND32i32: return convert32i32to32ri8(Inst, Cse523::AND32ri8);
                 //case Cse523::AND64i32: return convert64i32to64ri8(Inst, Cse523::AND64ri8);
                 //case Cse523::XOR16i16: return convert16i16to16ri8(Inst, Cse523::XOR16ri8);
                 //case Cse523::XOR32i32: return convert32i32to32ri8(Inst, Cse523::XOR32ri8);
                 //case Cse523::XOR64i32: return convert64i32to64ri8(Inst, Cse523::XOR64ri8);
                 //case Cse523::OR16i16:  return convert16i16to16ri8(Inst, Cse523::OR16ri8);
                 //case Cse523::OR32i32:  return convert32i32to32ri8(Inst, Cse523::OR32ri8);
                 //case Cse523::OR64i32:  return convert64i32to64ri8(Inst, Cse523::OR64ri8);
                 //case Cse523::CMP16i16: return convert16i16to16ri8(Inst, Cse523::CMP16ri8, true);
                 //case Cse523::CMP32i32: return convert32i32to32ri8(Inst, Cse523::CMP32ri8, true);
                 //case Cse523::CMP64i32: return convert64i32to64ri8(Inst, Cse523::CMP64ri8, true);
                 //case Cse523::ADD16i16: return convert16i16to16ri8(Inst, Cse523::ADD16ri8);
                 //case Cse523::ADD32i32: return convert32i32to32ri8(Inst, Cse523::ADD32ri8);
                 //case Cse523::ADD64i32: return convert64i32to64ri8(Inst, Cse523::ADD64ri8);
                 //case Cse523::SUB16i16: return convert16i16to16ri8(Inst, Cse523::SUB16ri8);
                 //case Cse523::SUB32i32: return convert32i32to32ri8(Inst, Cse523::SUB32ri8);
                 //case Cse523::SUB64i32: return convert64i32to64ri8(Inst, Cse523::SUB64ri8);
                 //case Cse523::ADC16i16: return convert16i16to16ri8(Inst, Cse523::ADC16ri8);
                 //case Cse523::ADC32i32: return convert32i32to32ri8(Inst, Cse523::ADC32ri8);
                 //case Cse523::ADC64i32: return convert64i32to64ri8(Inst, Cse523::ADC64ri8);
                 //case Cse523::SBB16i16: return convert16i16to16ri8(Inst, Cse523::SBB16ri8);
                 //case Cse523::SBB32i32: return convert32i32to32ri8(Inst, Cse523::SBB32ri8);
                 //case Cse523::SBB64i32: return convert64i32to64ri8(Inst, Cse523::SBB64ri8);
                 ...
                 //  if (Cse523II::isCse523_64ExtendedReg(Inst.getOperand(0).getReg()) ||
                 //      !Cse523II::isCse523_64ExtendedReg(Inst.getOperand(1).getReg()))
                 //    return false;
                 ...
                 //  if (Cse523II::isCse523_64ExtendedReg(Inst.getOperand(0).getReg()) ||
                 //      !Cse523II::isCse523_64ExtendedReg(Inst.getOperand(2).getReg()))
                 //    return false;
                 //  unsigned NewOpc;
                 //  switch (Inst.getOpcode()) {
                 //  default: llvm_unreachable("Invalid opcode");
...
MatchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
    ...
    Cse523Operand *Op = static_cast<Cse523Operand*>(Operands[0]);
    ...
    if (Op->getToken() == "fstsw" || Op->getToken() == "fstcw" ||
        ...
        delete Operands[0];
        Operands[0] = Cse523Operand::CreateToken(Repl, IDLoc);
    }
    ...
    if ((Match1 == Match_MnemonicFail) && (Match2 == Match_MnemonicFail) &&
        ...
        if (ErrorInfo != ~0U) {
            ...
            Cse523Operand *Operand = (Cse523Operand*)Operands[ErrorInfo];
...
bool Cse523AsmParser::ParseDirective(AsmToken DirectiveID) {
...
bool Cse523AsmParser::ParseDirectiveWord(unsigned Size, SMLoc L) {
    if (getLexer().isNot(AsmToken::EndOfStatement)) {
        for (;;) {
            const MCExpr *Value;
...
bool Cse523AsmParser::ParseDirectiveCode(StringRef IDVal, SMLoc L) {
//    if (IDVal == ".code16") {
//        Parser.Lex();
//        if (!is16BitMode()) {
//            SwitchMode(Cse523::Mode16Bit);
//            getParser().getStreamer().EmitAssemblerFlag(MCAF_Code16);
//        }
//    } else if (IDVal == ".code32") {
//        Parser.Lex();
//        if (!is32BitMode()) {
//            SwitchMode(Cse523::Mode32Bit);
//            getParser().getStreamer().EmitAssemblerFlag(MCAF_Code32);
//        }
//    } else if (IDVal == ".code64") {
//        Parser.Lex();
//        if (!is64BitMode()) {
//            SwitchMode(Cse523::Mode64Bit);
