//===-- Cse523CodeEmitter.cpp - Convert Cse523 code to machine code -------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the pass that transforms the Cse523 machine instructions into
// relocatable machine code.
//
//===----------------------------------------------------------------------===//

#define DEBUG_TYPE "cse523-emitter"
#include "Cse523.h"
#include "Cse523InstrInfo.h"
//#include "Cse523JITInfo.h"
#include "Cse523Relocations.h"
...
namespace {
    template<class CodeEmitter>
        class Emitter : public MachineFunctionPass {
            ...
            public:
            static char ID;
            explicit Emitter(Cse523TargetMachine &tm, CodeEmitter &mce)
            ...
            virtual const char *getPassName() const {
                return "Cse523 Machine Code Emitter";
            }
            ...
            unsigned getCse523RegNum(unsigned RegNo) const {
...
FunctionPass *llvm::createCse523JITCodeEmitterPass(Cse523TargetMachine &TM,
        JITCodeEmitter &JCE) {
...
bool Emitter<CodeEmitter>::runOnMachineFunction(MachineFunction &MF) {
    ...
    II = TM.getInstrInfo();
    TD = TM.getDataLayout();
    Is64BitMode = TM.getSubtarget<Cse523Subtarget>().is64Bit();
    IsPIC = TM.getRelocationModel() == Reloc::PIC_;
    ...
    do {
        ...
        for (MachineFunction::iterator MBB = MF.begin(), E = MF.end();
            ...
            for (MachineBasicBlock::iterator I = MBB->begin(), E = MBB->end();
                ...
                //if (Desc.getOpcode() == Cse523::MOVPC32r)
                //emitInstruction(*I, &II->get(Cse523::POP32r));
...
/// determineREX - Determine if the MachineInstr has to be encoded with a Cse523-64
...
static unsigned determineREX(const MachineInstr &MI) {
    ...
    if ((Desc.TSFlags & Cse523II::FormMask) == Cse523II::Pseudo)
        return 0;
    if (Desc.TSFlags & Cse523II::REX_W)
        REX |= 1 << 3;
    ...
    if (NumOps) {
        ...
        for (unsigned e = NumOps; i != e; ++i) {
            ...
            if (MO.isReg()) {
                ...
                if (Cse523II::isCse523NonExtLowByteReg(Reg))
                    REX |= 0x40;
            }
        }

        switch (Desc.TSFlags & Cse523II::FormMask) {
            case Cse523II::MRMSrcReg: {
                                          if (Cse523InstrInfo::isCse523_64ExtendedReg(MI.getOperand(0)))
                                              REX |= 1 << 2;
                                          i = isTwoAddr ? 2 : 1;
                                          for (unsigned e = NumOps; i != e; ++i) {
                                              const MachineOperand& MO = MI.getOperand(i);
                                              if (Cse523InstrInfo::isCse523_64ExtendedReg(MO))
                                                  REX |= 1 << 0;
                                          }
                                          break;
                                      }
            ...
            case Cse523II::MRMDestMem: {
                                           ...
                                           i = isTwoAddr ? 1 : 0;
                                           if (NumOps > e && Cse523InstrInfo::isCse523_64ExtendedReg(MI.getOperand(e)))
                                               REX |= 1 << 2;
                                           unsigned Bit = 0;
                                           for (; i != e; ++i) {
                                               const MachineOperand& MO = MI.getOperand(i);
...
void Emitter<CodeEmitter>::emitGlobalAddress(const GlobalValue *GV,
    ...
    if (Reloc == Cse523::reloc_picrel_word)
        RelocCST = PICBaseOffset;
    else if (Reloc == Cse523::reloc_pcrel_word)
        RelocCST = PCAdj;
    ...
    if (Reloc == Cse523::reloc_absolute_dword)
...
void Emitter<CodeEmitter>::emitExternalSymbolAddress(const char *ES,
        unsigned Reloc) {
    intptr_t RelocCST = (Reloc == Cse523::reloc_picrel_word) ? PICBaseOffset : 0;

    // Cse523 never needs stubs because instruction selection will always pick
    // an instruction sequence that is large enough to hold any address
    // to a symbol.
    // (see Cse523ISelLowering.cpp, near 2039: Cse523TargetLowering::LowerCall)
    bool NeedStub = false;
...
void Emitter<CodeEmitter>::emitRegModRMByte(unsigned ModRMReg,
    ...
    MCE.emitByte(ModRMByte(3, RegOpcodeFld, getCse523RegNum(ModRMReg)));
}
...
static bool gvNeedsNonLazyPtr(const MachineOperand &GVOp,
    ...
    if (TM.getSubtarget<Cse523Subtarget>().is64Bit() &&
            !TM.getSubtarget<Cse523Subtarget>().isTargetDarwin())
        return false;
...
void Emitter<CodeEmitter>::emitDisplacementField(const MachineOperand *RelocOp,
    ...
    unsigned RelocType = Is64BitMode ?
        (IsPCRel ? Cse523::reloc_pcrel_word : Cse523::reloc_absolute_word_sext)
...
void Emitter<CodeEmitter>::emitMemModRMByte(const MachineInstr &MI,
    ...
    if (BaseReg == Cse523::RIP ||
            (Is64BitMode && DispForReloc)) { // [disp32+RIP] in Cse523-64 mode
    ...
    if (BaseReg != 0 && BaseReg != Cse523::RIP)
        BaseRegNo = getCse523RegNum(BaseReg);
    ...
    if (// The SIB byte must be used if there is an index register.
        ...
        if (BaseReg == 0 ||          // [disp32]     in Cse523-32 mode
                BaseReg == Cse523::RIP) {   // [disp32+RIP] in Cse523-64 mode
            MCE.emitByte(ModRMByte(0, RegOpcodeField, 5));
            emitDisplacementField(DispForReloc, DispVal, PCAdj, true);
            return;
        }
    ...
    assert(IndexReg.getReg() != Cse523::RSP && "Cannot use ESP as index reg!");
    ...
    if (BaseReg == 0) {
        ...
        unsigned IndexRegNo;
        if (IndexReg.getReg())
            IndexRegNo = getCse523RegNum(IndexReg.getReg());
        else // Examples: [ESP+1*<noreg>+4] or [scaled idx]+disp32 (MOD=0,BASE=5)
            IndexRegNo = 4;
        emitSIBByte(SS, IndexRegNo, 5);
    } else {
        unsigned BaseRegNo = getCse523RegNum(BaseReg);
        unsigned IndexRegNo;
        if (IndexReg.getReg())
            IndexRegNo = getCse523RegNum(IndexReg.getReg());
        else
...
static const MCInstrDesc *UpdateOp(MachineInstr &MI, const Cse523InstrInfo *II,
        unsigned Opcode) {
...
static bool Is16BitMemOperand(const MachineInstr &MI, unsigned Op) {
    const MachineOperand &BaseReg  = MI.getOperand(Op+Cse523::AddrBaseReg);
    const MachineOperand &IndexReg = MI.getOperand(Op+Cse523::AddrIndexReg);
...
void Emitter<CodeEmitter>::emitOpcodePrefix(uint64_t TSFlags,
    ...
    if (((TSFlags & Cse523II::OpSizeMask) >> Cse523II::OpSizeShift) == Cse523II::OpSize16)
        MCE.emitByte(0x66);

    switch (Desc->TSFlags & Cse523II::OpPrefixMask) {
        case Cse523II::PD:   // 66
            MCE.emitByte(0x66);
            break;
        case Cse523II::XS:   // F3
    ...
    switch (Desc->TSFlags & Cse523II::OpMapMask) {
...
Emitter<CodeEmitter>::getVEXRegisterEncoding(const MachineInstr &MI,
    ...
    unsigned SrcRegNum = getCse523RegNum(MI.getOperand(OpNum).getReg());
...
void Emitter<CodeEmitter>::emitSegmentOverridePrefix(uint64_t TSFlags,
    ...
    switch (MI.getOperand(MemOperand+Cse523::AddrSegmentReg).getReg()) {
        default: llvm_unreachable("Unknown segment register!");
        case 0: break;
        case Cse523::CS: MCE.emitByte(0x2E); break;
        case Cse523::SS: MCE.emitByte(0x36); break;
        case Cse523::DS: MCE.emitByte(0x3E); break;
        case Cse523::ES: MCE.emitByte(0x26); break;
        case Cse523::FS: MCE.emitByte(0x64); break;
        case Cse523::GS: MCE.emitByte(0x65); break;
    }
}
...
void Emitter<CodeEmitter>::emitVEXOpcodePrefix(uint64_t TSFlags,
    ...
    bool HasVEX_4VOp3 = (TSFlags >> Cse523II::VEXShift) & Cse523II::VEX_4VOp3;
    ...
    if ((TSFlags >> Cse523II::VEXShift) & Cse523II::VEX_W)
        VEX_W = 1;

    if ((TSFlags >> Cse523II::VEXShift) & Cse523II::VEX_L)
        VEX_L = 1;

    switch (TSFlags & Cse523II::OpPrefixMask) {
        default: break; // VEX_PP already correct
        case Cse523II::PD: VEX_PP = 0x1; break; // 66
        case Cse523II::XS: VEX_PP = 0x2; break; // F3
        case Cse523II::XD: VEX_PP = 0x3; break; // F2
    }

    switch (TSFlags & Cse523II::OpMapMask) {
        default: llvm_unreachable("Invalid prefix!");
        case Cse523II::TB:   VEX_5M = 0x1; break; // 0F
        case Cse523II::T8:   VEX_5M = 0x2; break; // 0F 38
        case Cse523II::TA:   VEX_5M = 0x3; break; // 0F 3A
        case Cse523II::XOP8: VEX_5M = 0x8; break;
        case Cse523II::XOP9: VEX_5M = 0x9; break;
        case Cse523II::XOPA: VEX_5M = 0xA; break;
    }
    ...
    switch (TSFlags & Cse523II::FormMask) {
        ...
        case Cse523II::MRMDestMem: {
                                       ...
                                       //
                                       if (Cse523II::isCse523ExtendedReg(MI.getOperand(Cse523::AddrBaseReg).getReg()))
                                           VEX_B = 0x0;
                                       if (Cse523II::isCse523ExtendedReg(MI.getOperand(Cse523::AddrIndexReg).getReg()))
                                           VEX_X = 0x0;
        ...
        case Cse523II::MRMSrcMem:
                                   ...
                                   if (Cse523II::isCse523ExtendedReg(MI.getOperand(CurOp).getReg()))
                                       VEX_R = 0x0;
                                   CurOp++;
                                   ...
                                   if (Cse523II::isCse523ExtendedReg(
                                               MI.getOperand(MemOperand+Cse523::AddrBaseReg).getReg()))
                                       VEX_B = 0x0;
                                   if (Cse523II::isCse523ExtendedReg(
                                               MI.getOperand(MemOperand+Cse523::AddrIndexReg).getReg()))
                                       VEX_X = 0x0;

                                   if (HasVEX_4VOp3)
                                       VEX_4V = getVEXRegisterEncoding(MI, CurOp+Cse523::AddrNumOperands);
                                   break;
        case Cse523II::MRM0m: case Cse523II::MRM1m:
        case Cse523II::MRM2m: case Cse523II::MRM3m:
        case Cse523II::MRM4m: case Cse523II::MRM5m:
    ...
    if (Encoding == Cse523II::VEX && VEX_B && VEX_X && !VEX_W && (VEX_5M == 1)) {
        MCE.emitByte(0xC5);
    ...
    // 3 byte VEX prefix
    MCE.emitByte(Encoding == Cse523II::XOP ? 0x8F : 0xC4);
...
void Emitter<CodeEmitter>::emitInstruction(MachineInstr &MI,
    ...
    //    case Cse523::ADD16rr_DB:      Desc = UpdateOp(MI, II, Cse523::OR16rr); break;
    //    case Cse523::ADD32rr_DB:      Desc = UpdateOp(MI, II, Cse523::OR32rr); break;
    //    case Cse523::ADD64rr_DB:      Desc = UpdateOp(MI, II, Cse523::OR64rr); break;
    //    case Cse523::ADD16ri_DB:      Desc = UpdateOp(MI, II, Cse523::OR16ri); break;
    //    case Cse523::ADD32ri_DB:      Desc = UpdateOp(MI, II, Cse523::OR32ri); break;
    //    case Cse523::ADD64ri32_DB:    Desc = UpdateOp(MI, II, Cse523::OR64ri32); break;
    //    case Cse523::ADD16ri8_DB:     Desc = UpdateOp(MI, II, Cse523::OR16ri8); break;
    //    case Cse523::ADD32ri8_DB:     Desc = UpdateOp(MI, II, Cse523::OR32ri8); break;
    //    case Cse523::ADD64ri8_DB:     Desc = UpdateOp(MI, II, Cse523::OR64ri8); break;
    //    case Cse523::ACQUIRE_MOV8rm:  Desc = UpdateOp(MI, II, Cse523::MOV8rm); break;
    //    case Cse523::ACQUIRE_MOV16rm: Desc = UpdateOp(MI, II, Cse523::MOV16rm); break;
    //    case Cse523::ACQUIRE_MOV32rm: Desc = UpdateOp(MI, II, Cse523::MOV32rm); break;
    //    case Cse523::ACQUIRE_MOV64rm: Desc = UpdateOp(MI, II, Cse523::MOV64rm); break;
    //    case Cse523::RELEASE_MOV8mr:  Desc = UpdateOp(MI, II, Cse523::MOV8mr); break;
    //    case Cse523::RELEASE_MOV16mr: Desc = UpdateOp(MI, II, Cse523::MOV16mr); break;
    //    case Cse523::RELEASE_MOV32mr: Desc = UpdateOp(MI, II, Cse523::MOV32mr); break;
    //    case Cse523::RELEASE_MOV64mr: Desc = UpdateOp(MI, II, Cse523::MOV64mr); break;
    //}
    ...
    if (Desc->TSFlags & Cse523II::REP)
        MCE.emitByte(0xF3);

    // Emit the address size opcode prefix as needed.
    bool need_address_override;
    if (TSFlags & Cse523II::AdSize) {
    ...
    unsigned char BaseOpcode = Cse523II::getBaseOpcodeFor(Desc->TSFlags);
    switch (TSFlags & Cse523II::FormMask) {
        default:
            llvm_unreachable("Unknown FormMask value in Cse523 MachineCodeEmitter!");
...
//                                           emitConstant(0, Cse523II::getSizeOfImm(Desc->TSFlags));
//                                           // Remember PIC base.
//                                           PICBaseOffset = (intptr_t) MCE.getCurrentPCOffset();
//                                           Cse523JITInfo *JTI = TM.getJITInfo();
...
//
//                                   if (MO.isGlobal()) {
//                                       emitGlobalAddress(MO.getGlobal(), Cse523::reloc_pcrel_word,
//                                               MO.getOffset(), 0);
//                                       break;
//                                   }
//
//                                   if (MO.isSymbol()) {
//                                       emitExternalSymbolAddress(MO.getSymbolName(), Cse523::reloc_pcrel_word);
//                                       break;
//                                   }
//
//                                   // FIXME: Only used by hackish MCCodeEmitter, remove when dead.
//                                   if (MO.isJTI()) {
//                                       emitJumpTableAddress(MO.getIndex(), Cse523::reloc_pcrel_word);
//                                       break;
//                                   }
//
//                                   assert(MO.isImm() && "Unknown RawFrm operand!");
//                                   if (Opcode == Cse523::CALLpcrel32 || Opcode == Cse523::CALL64pcrel32) {
//                                       // Fix up immediate operand for pc relative calls.
//                                       intptr_t Imm = (intptr_t)MO.getImm();
//                                       Imm = Imm - MCE.getCurrentPCValue() - 4;
//                                       emitConstant(Imm, Cse523II::getSizeOfImm(Desc->TSFlags));
//                                   } else
//                                       emitConstant(MO.getImm(), Cse523II::getSizeOfImm(Desc->TSFlags));
//                                   break;
//                               }
//
//        case Cse523II::AddRegFrm: {
//                                      MCE.emitByte(BaseOpcode +
//                                              getCse523RegNum(MI.getOperand(CurOp++).getReg()));
//
//                                      if (CurOp == NumOps)
//                                          break;
//
//                                      const MachineOperand &MO1 = MI.getOperand(CurOp++);
//                                      unsigned Size = Cse523II::getSizeOfImm(Desc->TSFlags);
//                                      if (MO1.isImm()) {
...
//                                          rt = Cse523::reloc_absolute_dword;  // FIXME: add Cse523II flag?
//                                      if (MO1.isGlobal()) {
...
//                                               getCse523RegNum(MI.getOperand(SrcRegNum).getReg()));
//                                       CurOp = SrcRegNum + 1;
//                                       break;
//                                   }
//        case Cse523II::MRMDestMem: {
//                                       MCE.emitByte(BaseOpcode);
//
//                                       unsigned SrcRegNum = CurOp + Cse523::AddrNumOperands;
//                                       if (HasVEX_4V) // Skip 1st src (which is encoded in VEX_VVVV)
//                                           SrcRegNum++;
...
//                                          Cse523II::getSizeOfImm(Desc->TSFlags) : 0;
//                                      emitMemModRMByte(MI, FirstMemOp,
//                                              getCse523RegNum(MI.getOperand(CurOp).getReg()),PCAdj);
...
//                                                            ++CurOp;
//                                                        MCE.emitByte(BaseOpcode);
//                                                        uint64_t Form = (Desc->TSFlags & Cse523II::FormMask);
//                                                        emitRegModRMByte(MI.getOperand(CurOp++).getReg(),
...
//                                                        unsigned rt = Is64BitMode ? Cse523::reloc_pcrel_word
//                                                            : (IsPIC ? Cse523::reloc_picrel_word : Cse523::reloc_absolute_word);
//                                                        if (Opcode == Cse523::MOV64ri32)
//                                                            rt = Cse523::reloc_absolute_word_sext;  // FIXME: add Cse523II flag?
//                                                        if (MO1.isGlobal()) {
//                                                            bool Indirect = gvNeedsNonLazyPtr(MO1, TM);
//                                                            emitGlobalAddress(MO1.getGlobal(), rt, MO1.getOffset(), 0,
//                                                                    Indirect);
...
//                                                        if (HasVEX_4V) // Skip the register dst (which is encoded in VEX_VVVV).
//                                                            ++CurOp;
//                                                        intptr_t PCAdj = (CurOp + Cse523::AddrNumOperands != NumOps) ?
//                                                            (MI.getOperand(CurOp+Cse523::AddrNumOperands).isImm() ?
//                                                             Cse523II::getSizeOfImm(Desc->TSFlags) : 4) : 0;
//
//                                                        MCE.emitByte(BaseOpcode);
//                                                        uint64_t Form = (Desc->TSFlags & Cse523II::FormMask);
...
//                                                    break;
    ...
    while (CurOp != NumOps && NumOps - CurOp <= 2) {
        ...
        if ((TSFlags >> Cse523II::VEXShift) & Cse523II::VEX_I8IMM) {
            const MachineOperand &MO = MI.getOperand(HasMemOp4 ? MemOp4_I8IMMOperand
                    : CurOp);
            ++CurOp;
            unsigned RegNum = getCse523RegNum(MO.getReg()) << 4;
            ...
            emitConstant(MI.getOperand(CurOp++).getImm(),
                    Cse523II::getSizeOfImm(Desc->TSFlags));
        }
    }
